...
Light::Light(std::pair<std::ofstream, uint32_t>&& lcd_backlight,
             ...
             std::ofstream&& red_delay_off, std::ofstream&& green_delay_off, std::ofstream&& blue_delay_off,
             std::ofstream&& red_delay_on, std::ofstream&& green_delay_on, std::ofstream&& blue_delay_on)
    : mLcdBacklight(std::move(lcd_backlight)),
      mRedLed(std::move(red_led)),
      mGreenLed(std::move(green_led)),
      mBlueLed(std::move(blue_led)),
      mRedDelayOff(std::move(red_delay_off)),
      mGreenDelayOff(std::move(green_delay_off)),
      mBlueDelayOff(std::move(blue_delay_off)),
      mRedDelayOn(std::move(red_delay_on)),
      mGreenDelayOn(std::move(green_delay_on)),
      mBlueDelayOn(std::move(blue_delay_on)) {
...
void Light::setSpeakerBatteryLightLocked() {
    if (isLit(mNotificationState)) {
        ...
        mBlueLed << 0 << std::endl;
        mRedDelayOn << 0 << std::endl;
        mGreenDelayOn << 0 << std::endl;
        mBlueDelayOn << 0 << std::endl;
        mRedDelayOff << 0 << std::endl;
        mGreenDelayOff << 0 << std::endl;
        mBlueDelayOff << 0 << std::endl;
    }
}
...
void Light::setSpeakerLightLocked(const LightState& state) {
    ...
    mRedDelayOn << (blink && red ? onMs : 0) << std::endl;
    mRedDelayOff << (blink && red ? offMs : 0) << std::endl;
    mGreenDelayOn << (blink && green ? onMs : 0) << std::endl;
    mGreenDelayOff << (blink && green ? offMs : 0) << std::endl;
    mBlueDelayOn << (blink && blue ? onMs : 0) << std::endl;
    mBlueDelayOff << (blink && blue ? offMs : 0) << std::endl;
